...
uniform samplerCube diffuseradianceMap;
uniform samplerCube diffuseradianceMap_diffuseIrradiance;
...
void main()
{
  ...
  for(int i=0; i< MAX_LIGHTS; i++){
    ...
    if(length(lights[i].color)==0)
      ...
      vec3 kD = 1.0 - kS;
      vec3 irradiance = texture(diffuseradianceMap_diffuseIrradiance, N).rgb;
